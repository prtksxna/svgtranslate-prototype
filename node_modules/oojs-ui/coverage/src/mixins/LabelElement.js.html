...
OO.ui.mixin.LabelElement.prototype.setLabel = function ( label ) {
	...
	label = ( ( typeof label === 'string' || label instanceof jQuery ) &amp;&amp; label.length ) || ( label instanceof OO.ui.HtmlSnippet &amp;&amp; <span class="branch-4 cbranch-no" title="branch not covered" >label.toString().length </span>) ? label : null;
&nbsp;
	if ( this.label !== label ) {
		if ( this.$label ) {
...
OO.ui.mixin.LabelElement.prototype.setLabelContent = function ( label ) {
	if ( typeof label === 'string' ) {
		<span class="missing-if-branch" title="if path not taken" >I</span>if ( label.match( /^\s*$/ ) ) {
			// Convert whitespace only string to a single non-breaking space
<span class="cstat-no" title="statement not covered" >			this.$label.html( '&amp;nbsp;' );</span>
		} else {
			this.$label.text( label );
		}
	} else <span class="missing-if-branch" title="if path not taken" >I</span>if ( label instanceof OO.ui.HtmlSnippet ) {
<span class="cstat-no" title="statement not covered" >		this.$label.html( label.toString() );</span>
